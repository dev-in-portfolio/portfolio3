...
# - exit 1: run build
#
# This script is strict by default to avoid burning deploys.
...
if [[ "$context" == "branch-deploy" ]]; then
  if [[ -z "$allow_branches_raw" ]]; then
    echo "netlify-ignore: skipping branch-deploy for '$branch' (no ALLOWED_BRANCH_DEPLOYS set)."
    exit 0
  fi
  ...
  for allowed in "${allowed_branches[@]}"; do
    ...
    if [[ -n "$allowed_trimmed" && "$branch" == "$allowed_trimmed" ]]; then
      echo "netlify-ignore: allowing branch-deploy for '$branch'."
      exit 1
    fi
  done

  echo "netlify-ignore: skipping branch-deploy for '$branch' (not in ALLOWED_BRANCH_DEPLOYS)."
  exit 0
fi

echo "netlify-ignore: skipping context '$context' by default."
exit 0
